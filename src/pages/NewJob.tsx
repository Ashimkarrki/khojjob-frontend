...
const NewJob = () => {
  ...
  `;
  const { data, loading } = useQuery(GET_NEW_JOB);
  const [jobs, setJobs] = useState<propJobType[]>([]);

  useEffect(() => {
    setJobs(data?.getNewJob || []);
  }, [data]);
  console.log(jobs);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="text-center">
            ...
            <p className="text-xl text-gray-200 mb-8">
              Discover the latest job openings in Nepal's tech industry
            </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="space-y-6">
          {jobs?.map((job: propJobType) => {
            return (
              <JobItem key={job.jid} job={job} from="new" setJobs={setJobs} />
            );
          })}
        </div>
